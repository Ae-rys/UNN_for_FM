...
def sigma_max(W: torch.Tensor) -> torch.Tensor:
    ...
    if W.dim() == 4:
        # Conv kernel: (out, in, kH, kW) → reshape to 2-D matrix
        mat = W.view(W.shape[0], -1)
    else:
        mat = W
    return torch.linalg.matrix_norm(mat, ord=2)  # largest singular value
...
class SmallUNet(nn.Module):
    ...
    def forward(self, xt_t):
        ...
        if t.dim() > 2:
            t = t.squeeze(-1)
        t_emb = self.alpha * self.time_scaling(t).view(batch_size, -1, 1, 1)
...
class DoubleConvTime(nn.Module):
    ...
    def forward(self, x, t):
        t_emb = self.alpha * self.time_scaling(t)
        h = self.silu(self.gn1(self.conv1(x)))
        h = h + t_emb[:, :, None, None]
        h = self.silu(self.gn2(self.conv2(h)))
        return h
...
class DFB_Iteration(nn.Module):
    def __init__(self, dim, prox, alpha, W=None, V=None, learned_prox=True, version="LFO"):
        super().__init__()
        ...
        if version == "LFO":
            ...
            self.V_weight = self.W_weight.T
            self.tau = 1.99 * (1 / sigma_max(self.W_weight) ** 2)
        if not learned_prox:
            self.time_scaling = nn.Sequential(
    ...
    def forward(self, x, u, z, t):
        mu_t   = t if self.learned_prox else self.time_scaling(t)
        x_next = z - F.linear(u, self.V_weight, None)
        step   = self.tau * F.linear(x_next, self.W_weight, None)
        u_next = self.prox(torch.cat((u + step, self.alpha * mu_t), dim=-1))
...
class DFB_UNN(nn.Module):
    def __init__(self, dim, K=10, alpha_init=1.0, learned_prox=False, w=32, version="LFO"):
        ...
        self.layers = nn.ModuleList([
            DFB_Iteration(dim, self.proxs[i], self.alpha, version=version) for i in range(K)
        ])
...
class DiFB_UNN(nn.Module):
    def __init__(self, dim, K=10, alpha_init=1.0, w=32, version="LFO"):
        super().__init__()
        self.dim   = dim
        self.K     = K
        self.alpha = nn.Parameter(torch.tensor(alpha_init))
        self.proxs = nn.ModuleList([MLP(dim=dim, time_varying=True, w=w) for _ in range(K)])
        self.layers = nn.ModuleList([
            DFB_Iteration(dim, self.proxs[i], alpha=self.alpha, version=version) for i in range(K)
        ])
...
class ConvDFB_Iteration(nn.Module):
    def __init__(self, internal_channel, alpha, version="LFO"):
        ...
        if version == "LFO":
            ...
            self.V_weight = self.W_weight
            self.tau = 1.99 * sigma_max(self.W_weight)
    ...
    def forward(self, u, z, t):
        ...
        step   = self.tau * F.conv2d(x_next, self.V_weight, padding=1)
...
class SharedConvDFB_UNN(nn.Module):
    def __init__(self, dim=784, K=10, alpha_init=1.0, internal_channel=64,
        ...
        if version == "LFO":
            ...
            self.shared_V = self.shared_W
            self.tau = 1.99 * (1 / sigma_max(self.shared_W) ** 2)
        self.alpha = nn.Parameter(torch.tensor(alpha_init))
        if use_Unet:
            self.prox = SmallUNet(
    ...
    def forward(self, xt_t, n_iter=None):
        ...
        for _ in range(iters):
            x_next = z - F.conv_transpose2d(u, self.shared_V, padding=1)
            step   = self.tau * F.conv2d(x_next, self.shared_W, padding=1)
            if self.use_Unet:
...
class CP_Iteration(nn.Module):
    def __init__(self, dim, prox_dual, theta=1.0, alpha=1.0, version="LFO"):
        super().__init__()
        ...
        if version == "LFO":
            ...
            self.V_weight = self.W_weight.T
            # FIX: was spectral_norm(W_weight) — undefined local variable
            self.tau = 0.99 * (1 / self.sigma) * (1 / sigma_max(self.W_weight) ** 2)
        self.time_scaling = nn.Sequential(
            nn.Linear(1, 32), nn.SiLU(), nn.Linear(32, 1),
        )
        self.alpha     = alpha
    ...
    def forward(self, x, x_bar, u, z, t):
        mu_t    = self.alpha * self.time_scaling(t)
        sigma_t = self.sigma * mu_t
        tau_t   = self.tau   * mu_t
        primal_input = x - tau_t * F.linear(u, self.V_weight, None)
        x_next  = (primal_input + tau_t * z) / (1 + tau_t)
        dual_step = sigma_t * F.linear(x_bar, self.W_weight, None)
...
class CP_UNN(nn.Module):
    def __init__(self, dim, K=10, alpha_init=1.0, w=32, version="LFO"):
        ...
        self.layers    = nn.ModuleList([
            CP_Iteration(dim, self.prox_list[i], alpha=self.alpha, version=version) for i in range(K)
        ])
...
class ConvCP_Iteration(nn.Module):
    def __init__(self, internal_channels=64, alpha=1.0, version="LFO"):
        ...
        self.prox_dual = DoubleConvTime(
            ...
            embed_dim=internal_channels // 2, alpha=self.alpha,
        )

    def forward(self, x, x_bar, u, z, t_prox):
        mu_t    = self.time_scaling(t_prox[:, None]).view(-1, 1, 1, 1)
        sigma_t = self.sigma * mu_t
        tau_t   = self.tau   * mu_t
        grad_u  = F.conv_transpose2d(u, self.V_weight, padding=1)
        primal_input = x - tau_t * grad_u
        x_next  = (primal_input + tau_t * z) / (1 + tau_t)
        dual_step = F.conv2d(x_bar, self.W_weight, padding=1)
        u_next  = self.prox_dual(u + sigma_t * dual_step, t_prox)
        x_bar_next = 2 * x_next - x
        return x_next, x_bar_next, u_next
...
class SharedConvCP_UNN(nn.Module):
    def __init__(self, dim=784, K=10, alpha_init=1.0, internal_channel=64,
        ...
        self.sigma    = nn.Parameter(torch.tensor(0.5))
        if version == "LFO":
            # FIX: was torch.randn(internal_channels, ...) — undefined variable
            self.shared_V = nn.Parameter(torch.randn(internal_channel, 1, 3, 3) * 0.05)
            self.tau = nn.Parameter(torch.tensor(0.5))
        else:
            self.shared_V = self.shared_W
            # FIX: was spectral_norm(shared_W) — undefined local variable
            self.tau = 0.99 * (1 / self.sigma) * (1 / sigma_max(self.shared_W) ** 2)
        self.alpha = nn.Parameter(torch.tensor(alpha_init))
        self.time_scaling = nn.Sequential(
            nn.Linear(1, 32), nn.SiLU(), nn.Linear(32, 1),
        )
        if use_Unet:
            ...
            self.prox = DoubleConvTime(
                in_ch=internal_channel, out_ch=internal_channel,
                embed_dim=internal_channel // 2, alpha=alpha_init,
            )
        self.use_Unet = use_Unet
    ...
    def forward(self, xt_t, n_iter=None):
        ...
        mu_t    = self.alpha * self.time_scaling(t).view(batch_size, 1, 1, 1)
        tau_t   = self.tau   * mu_t
        sigma_t = self.sigma * mu_t
        for _ in range(iters):
            grad_u       = F.conv_transpose2d(u, self.shared_V, padding=1)
            primal_input = x - tau_t * grad_u
            x_next       = (primal_input + tau_t * z) / (1 + tau_t)
            dual_step    = F.conv2d(x_bar, self.shared_W, padding=1)
            if self.use_Unet:
                u_flat    = (u + sigma_t * dual_step).view(batch_size, -1, 784)
                # FIX: was t_scaled — undefined variable; use t directly
                ut_t_flat = torch.cat(
                    [u_flat, t[:, 0].view(batch_size, 1, 1).expand(batch_size, self.internal_channel, 1)],
                    dim=-1,
                )
                ...
                # FIX: was t_scaled — undefined variable; use t directly
                u = self.prox(u + sigma_t * dual_step, t)
            x_bar = 2 * x_next - x
            x     = x_next
        return (x - z).view(batch_size, -1)
