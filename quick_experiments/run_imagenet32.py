...
def build_experiments(device, K=10, ic=256, kernel=9):
    return [
        ...
        dict(
            ...
            build = lambda: UNetModel(dim=(CHANNELS, IMG_SIZE, IMG_SIZE),
                                      num_channels=64, num_res_blocks=3).to(device),
        ),
        # ---- La variante : ConvScCP pixel RGB (recette MNIST gagnante, x-pred) ----
        dict(
            name  = f"ConvScCP_UNN_rgb_k{kernel}_K{K}_ic{ic}_L1_LNO",
...
def generate(model, name, device, n=8, seed=0):
    ...
    # meme test que train_one (`name.startswith("UNet")`) : tout UNetModel prend
    # une entree image et l'appel model(t, x). Un test d'egalite stricte sur
    # "UNet_torchCFM_baseline" faisait passer les autres UNet par le chemin
    # vectoriel, donc plantait des le premier echantillonnage.
...
def train_one(model, name, train_loader, device, run_dir, nb_epochs, lr, coupling="indep",
              save_model=False):
    ...
    # Couplage OT corrige (le bug de desappariement est fixe : on reconstruit la paire
    # coherente x1 = xt + ut(1-t) plus bas). L'ancienne note "OT casse ScCP" etait un artefact.
    ...
    for epoch in range(nb_epochs):
        ...
        for x1_img, _ in train_loader:
            ...
            if is_unet:
                ...
                if predicts_x1:
                    ...
                    # (fix desappariement ; en indep c'est identique au x1 d'origine).
                    x1_tgt = xt + ut * (1 - t.view(-1, 1))
                    # loss CFM exacte = MSE(out, x1) ponderee 1/(1-t)^2 (espace vitesse)
                    loss = torch.mean((out - x1_tgt) ** 2 / torch.clamp((1 - t.view(-1, 1)) ** 2, min=0.05))
                else:
                    loss = torch.mean((out - ut) ** 2)
        ...
        if epoch % 5 == 0 or epoch == nb_epochs - 1:
            ...
            if save_model:
                # ecrasement a chaque checkpoint : un run long reste recuperable
                ckpt = os.path.join(run_dir, "model.pt")
                torch.save({"name": name, "epoch": epoch + 1,
...
def main():
    ...
    p.add_argument("--coupling",    type=str, default="indep", choices=["indep", "ot"],
                   help="Couplage Flow-Matching : 'indep' (defaut) ou 'ot' (OT, bug corrige).")
    p.add_argument("--kernel",      type=int, default=9,  help="Taille noyau W du ConvScCP (9=recette MNIST, 3=local/Kamb).")
    p.add_argument("--K",           type=int, default=10, help="Nb d'iterations depliees du ConvScCP.")
    p.add_argument("--ic",          type=int, default=256, help="Canaux duals (internal_channel).")
    p.add_argument("--device",      type=str, default="cuda:0", help="Périphérique (cpu ou cuda).")
    p.add_argument("--save-model",  action="store_true",
                   help="Sauve le state_dict dans <results-dir>/<name>/model.pt "
                        "(reecrit a chaque epoch de checkpoint).")
    args = p.parse_args()
