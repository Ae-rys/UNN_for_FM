...
primal iterations x^[0], ..., x^[K] of a DFB_UNN to take steps of equal size:
...
Usage
-----
    python run_2moons_step.py [--epochs N] [--lambda-step L] [--results-dir DIR]
"""
...
# ---------------------------------------------------------------------------
# Step-uniformity regularizer
# ---------------------------------------------------------------------------

def step_uniformity_loss(x_traj):
    """Variance (over k) of the primal step sizes ||x^[k+1] - x^[k]||, averaged over the batch.

    x_traj: list of K+1 tensors of shape (B, dim), as returned by
    model(..., return_traj=True).
    """
    steps = torch.stack(
        [torch.norm(x_traj[k + 1] - x_traj[k], dim=-1) for k in range(len(x_traj) - 1)],
        dim=1,
    )  # (B, K)
    return steps.var(dim=1, unbiased=False).mean()
...
def main():
    parser = argparse.ArgumentParser(description="FM on 2-moons with primal step-uniformity regularization.")
    parser.add_argument("--epochs",      type=int,   default=100)
    parser.add_argument("--results-dir", type=str,   default="results_2moons_step")
    ...
    if not args.no_baseline:
        configs.append((f"DFB_UNN_{args.version}_step0", 0.0))
    configs.append((f"DFB_UNN_{args.version}_step{args.lambda_step}", args.lambda_step))
