...
from flops_utils import (
    flops_unet_model, flops_vector_model, write_train_time, write_velocity_flops,
)
...
def train_mnist(
    ...
    FM        = (ExactOptimalTransportConditionalFlowMatcher(sigma=0) if coupling == "ot"
                 else ConditionalFlowMatcher(sigma=0))
    ...
    # ---- FLOPs d'une evaluation de vitesse (batch 1) -> parametres.txt ----
    if model_name == "UNet_torchCFM_baseline" and not latent:
        flops = flops_unet_model(model, (1, 28, 28), device)
    else:
        flops = flops_vector_model(model, lat_dim if latent else 784, device)
    write_velocity_flops(run_dir, flops)
    ...
    for epoch in range(nb_epochs):
        ...
        for x1_batch, _ in tqdm(train_loader, desc=f"Epoch {epoch+1}/{nb_epochs}", leave=False):
            ...
            if getattr(model, "predicts_x1", False):
                ...
                if x1_weight == "uniform":
                    ...
                    w = 1.0 / torch.clamp(1.0 / omt2, max=min_snr_gamma)       # poids vitesse plafonné à gamma
                else:  # "invsq" : legacy 1/(1-t)² (EXPLOSE près de t=1)
                    w = 1.0 / torch.clamp(omt2, min=0.05 ** 2)
        ...
        # temps cumule tenu a jour a chaque epoch : reste correct meme si le run est tue
